...
■ How（どうやって） = 絶対禁止
  ...
  ユーザーがHowを言った場合 → 記録のみ。深掘りせず、ユーザにHowの話をここで混ぜてはいけないということを伝え、Why/Who/When/Whereに戻す。
...
■ Plannerは毎ターン、以下を実行してから次の質問を生成する:
  ...
  2b. **最初のユーザーメッセージ(anchor)にも4Wシグナル検出を適用**
     - 「私専用の壁打ちAIを作りたい」→ Who:「私専用=自分が使う」を即resolved
     - 「自分専用の〜」→ Who resolved
     - anchorに含まれる情報を後から聞くのは愚問
  3. 吸収した情報を confirmed_insights に追加する
  4. 吸収済みの内容と同じ or 似た質問は絶対に生成しない
  5. **既にユーザーが答えた情報を再確認する質問は絶対禁止**
     - 「私専用の」と言った後に「誰が使う？」は愚問
     - 「要件定義を作るとき」と言った後に「どんな企画で？」は重複
     - 「PdMとして」と言った後に「誰が使う？」は重複
  6. ユーザーがHowに言及した場合 → 記録のみ。深掘りせず、ユーザーにHowの話をここで混ぜてはいけないことを伝え、Why/Who/When/Whereに戻す
...
MGU ≥ 80% または 3次元以上resolved（Whyが明確） → 構造的まとめ生成
  根拠: ハーバート・サイモンの限定合理性（十分な解で意思決定）
  出力: Interviewerではなく構造的まとめプロンプト(buildSummaryPrompt)を使用
  ※ これ以上質問を続けるとHow接近のリスクが高まるため、まとめに入る
```

**構造的まとめ生成フェーズ（Phase 0.5）**:

```
■ 発動条件:
  - 3次元以上がresolved かつ Whyがresolvedまたはpartial
  - または MGU ≥ 80 かつ question_tree全体がallResolved
  - **ただし turn < 3 では発動しない**（LLMのdimension_status誤判定対策）

■ まとめのフォーマット:
  「ここまでの話を整理するね。」から始めて以下の構造で出力:
  1. あなたが解決したいペイン（課題）
     - ユーザーの言葉を引用しながら課題を具体的・明確に書く
  2. 背景（なぜそのペインが生まれるか）
     - Whyの回答から課題の構造的な理由を整理
  3. 利用者と利用場面
     - Who: 誰が使うか
     - When: どういう場面で使うか
  4. まとめ1文
...
■ open（question_type="open" または 新しい次元への質問）：情報収集フェーズ
  ...
  **裸の質問禁止（必須）**:
  ...
  例:
    OK → 「壁打ちAI作りたいんだ！なんで壁打ちAIを作ろうと思ったの？」
    OK → 「どういう場面で壁打ちAIを使いたいの？」
    NG → 「きっかけは？」← 裸の質問。何のきっかけか不明
...
■ hypothesis（question_type="hypothesis" かつ 同じ次元内の深掘り）：仮説確認フェーズ
  ...
  例:
    ユーザー発言1: 「壁打ちAIを作りたい」
    ユーザー発言2: 「要件定義でHowから入っちゃって、Whyを言語化できない」
    OK → 「つまり、要件定義を見せる相手にちゃんと"なぜ作るのか"を
           インプットした上で、Howに繋げたい。だから壁打ちでWhyを言語化したいってこと？」
    NG → 「つまり、Whyを重視することで明確な方向性を持たせたいってこと？」
          ← 浅い。ユーザーの発言を繋いでいない。ただのパラフレーズ。
    NG → 「なるほど、それはとても大変でしたね。」（感情語・評価語禁止）
    NG → 「つまり、どのような機能が必要ですか？」（つまり+How誘導質問は絶対禁止）

  **hypothesis の深度ルール（最重要）**:
  - **ユーザーの過去の発言を繋いで推論した仮説を出すこと**
  - 浅いパラフレーズ（言い直しただけ）は禁止
  - 「ユーザーが言ったAと、さっき言ったBを繋ぐと、Cということ？」の形
  - 1ステップ先の推論を含める（ユーザーが言っていない「だからこうしたいんだよね？」を加える）
...
■ 共通制約（全フェーズ）:
  ...
  **フレンドリーさ（必須）**:
  - 先輩が後輩と雑談しながら壁打ちしている温度感
  - 「〜ですか？」より「〜なの？」「〜ってこと？」
  - 冷たい事務的な質問ではなく、一緒に考えてる感じを出す
  
  **共感リアクション（必須）**:
  - ユーザーの発言に対して**文脈を踏まえた共感**を入れてから質問する
  - 「いいね」だけのリアクションは禁止。**なぜいいのか、文脈を繋いで理由を添える**
  - さらに**AIがどう理解しているか（見解）**も簡潔に示す
  - 淡々と質問だけ続けるのはユーザーにストレスを与える
  
  構造: [共感 + 文脈を踏まえた理由] → [AI見解] → [自然に繋がる質問]
  
  **会話の自然な繋がり（必須）**:
  - 共感パートと質問パートが分離してはいけない
  - 共感の流れの中で、自然に次の問いに繋がること
  - 「ちなみに」「そういえば」で急にトピックを変えるのは不自然
  - 共感で話した内容から「だからこそ」「そうなると」で質問に繋げる
  
  例（ユーザーが「Howから入っちゃう」と答えた後）:
    OK → 「あー、それめっちゃわかる！要件定義でいきなりHow書き始めちゃうと、
           結局なんで作るのかが曖昧になるんだよね。
           そうなると、その壁打ちってどういう場面で一番使いたくなる？」
    NG → 「あー、わかるわかる！...ちなみに、誰が使うの？」← 共感と質問が別人格
    NG → 「なるほど、いいですね！それは誰が使う想定ですか？」← 文脈なし+人格切替
    NG → 「それは誰が使う想定ですか？」← 共感ゼロ
