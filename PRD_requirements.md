...
| 認知フィルタリング | Grice's Maxims | How/What語を除外し Whyのみ追跡 |
...
Role B は Role A の `question_type`（MGUから決定）と `next_question_focus`（深掘りすべき層）を受け取り、以下のルールで発話を生成する。

```
■ MGU 0〜59%（question_type = "open"）：情報収集フェーズ
  ─ 先輩は何もまだわかっていない状態 ─
  ...
  トーン: 率直・シンプル・一文
  例:
    NG → 「技術的なボトルネックですか、それとも組織の問題が大きいですか？」
         （MGUが低い段階で仮説を提示 → Role Eが検出・差し戻し）
    OK → 「なんで作ろうと思ったの？」
    OK → 「要件定義を読み解けないって、具体的にどういう状況？」

■ MGU 60〜79%（question_type = "hypothesis"）：仮説確認フェーズ
  ─ 先輩はある程度わかってきた状態 ─
...
■ 共通制約（全フェーズ）:
  - 1ターン1質問（ミラーの法則を必ず守る）
  - JSON・スコア・メタ情報の漏洩禁止
  - How/What語の混入禁止（cognitive_filter に準拠）
...
1. **AIにプロンプトを大量に書かない** — 役割ごとに「1つの鋭い指示」を与える
2. **状態はDBに持つ** — AIへの伝言ゲームを排除する
3. **ユーザーは最初の1文だけ書けばいい** — あとはAIが引き出す
4. **情報のバトンリレーは完全であること** — どこかで欠損したらゴールに届かない
