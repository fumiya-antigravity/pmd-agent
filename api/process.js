...
2. **複数次元同時吸収**: 回答に複数次元の情報が含まれていたら、全て同時に吸収する
   - 例: 「要件定義を作るときに、Howから入っちゃって」
     → Why:「Howから入るのを防ぎたい」を吸収しresolved
     → When:「要件定義を作るとき」を吸収しresolved
   - 例: 「PdMとして自分一人で使いたい」
     → Who:「PdM、自分一人」を吸収しresolved
...
6. **既にユーザーが答えた情報を再確認する質問は絶対禁止**
   - 「要件定義を作るとき」と言った後に「どんな企画で？」と聞くのは重複
