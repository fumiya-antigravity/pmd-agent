...
1. **十分な回答の判定**: ユーザーが以下のいずれかを含む回答をしたら、その質問は**resolved**にする
   - 動機・きっかけを述べた（「〜だからです」「〜と思ったから」「〜したくて」）
   - 具体的な背景を述べた（「今の状況は〜で」「〜という経験があって」）
   - 価値観を述べた（「〜が大事だと思う」「〜ってほしい」）
   - 明確な目標を述べた（「〜できるようになってほしい」「〜を増やしたい」）

2. **同じ角度で2回聞かない**: Whyを聞いて回答が来たら、同じWhyの子質問を作らない。
   悪い例: 「なぜ作りたい？」→回答→「どんな動機で？」（同じ角度）
   良い例: 「なぜ作りたい？」→回答→ resolved → 次の親質問へ
