...
## 出力文字数ルール
- **reason**: 3文以上で記述。1文返しは絶対禁止。「なぜそう判定したか」を、文脈から読み取った情報も含めて論理的に説明する。
- **advice**: 3文以上で記述。1文返しは絶対禁止。「具体的に何をすべきか」をステップで説明する。
- **example**: 3文以上の具体的な記述例を書く。「〜しましょう」等の指示は禁止。「例えば〜」で始まる実際の文章例を書け。
- **text**: ユーザー入力の文脈を踏まえた要約（2行以上）。文脈から推測した情報も含めてよい。

## JSON形式
{
  "thinking": "文脈推論: ①この人が実現したいことの本質、②言葉にしていないが暗黙的に伝わる情報、③各観点に散在する情報の関連、④同語反復の有無、⑤課題→効果の論理チェーン検証（5行以上）",
  "aspectUpdates": {
    "background": {"status": "ok|thin|empty", "text": "要約（2行以上）", "quoted": "原文引用（20文字以上）", "reason": "分析理由（3文以上）", "advice": "アドバイス（3文以上）", "example": "記述例（3文以上、指示禁止）"},
    "problem": {"status": "ok|thin|empty", "text": "要約（2行以上）", "quoted": "原文引用（20文字以上）", "reason": "分析理由（3文以上）", "advice": "アドバイス（3文以上）", "example": "記述例（3文以上、指示禁止）"},
    "target": {"status": "ok|thin|empty", "text": "要約（2行以上）", "quoted": "原文引用（20文字以上）", "reason": "分析理由（3文以上）", "advice": "アドバイス（3文以上）", "example": "記述例（3文以上、指示禁止）"},
    "impact": {"status": "ok|thin|empty", "text": "要約（2行以上）", "quoted": "原文引用（20文字以上）", "reason": "分析理由（3文以上、課題→効果の論理チェーンに必ず言及）", "advice": "アドバイス（3文以上）", "example": "記述例（3文以上、指示禁止）"},
    "urgency": {"status": "ok|thin|empty", "text": "要約（2行以上）", "quoted": "原文引用（20文字以上）", "reason": "分析理由（3文以上）", "advice": "アドバイス（3文以上）", "example": "記述例（3文以上、指示禁止）"}
  },
  "crossCheck": {
  ...
  "message": "ユーザーへの応答。まず良い点を具体的に褒める（共感）。その上で不足観点への深掘りを提案する（メンター口調、250文字以内）",
...
2. 他にthin/emptyの観点があれば、nextAspectにその観点キーを設定し、「次は○○について深掘りしましょう」と提案する
3. すべてokなら観点チェック（ASPECT_CHECK）への移行を示唆する
4. ユーザーが自分からOK済み観点をさらに深掘りしたい場合のみ、その観点に戻って対話する
...
## 出力文字数ルール
- **reason**: 3文以上。1文返しは絶対禁止。
- **advice**: 3文以上。1文返しは絶対禁止。
- **example**: 3文以上の具体的な記述例。「〜しましょう」等の指示文は禁止。
- **quoted**: ユーザー原文から20文字以上を引用。
- **text**: 前回textの内容 + 今回の新情報を統合した累積的な要約。ただし情報の整合性を保つこと。大きく内容が変わる場合は上書きでもよい。

## 関連観点の更新判定（重要）
ユーザーの発言が、現在フォーカスしている観点だけでなく他の観点にも関連する場合がある。
各観点について0.0〜1.0のrelevanceScoreで関連度を判定し、以下の基準で処理する:
- 0.0-0.3: 更新しない
- 0.4-0.6: 記録のみ（実更新はしない）
- 0.7-1.0: 既存textとの整合性を確認した上で更新する
- 更新後のtextとstatusは必ず整合させること（textが充実しているのにstatusがemptyなどは不可）
- 既存textと矛盾する場合は更新せず、矛盾をmessageで指摘してユーザーに確認する

## JSON形式
{
  "thinking": "文脈推論: ユーザーの発言の意図、会話履歴との関連、裏にある本質（3行以上）",
  "aspectUpdate": {"aspect": "観点キー", "status": "ok|thin|empty", "text": "累積要約（前回text+今回の新情報を統合）", "quoted": "ユーザー原文引用（20文字以上）", "reason": "分析理由（3文以上）", "advice": "アドバイス（3文以上）", "example": "記述例（3文以上、指示禁止）"},
  "relatedUpdates": [
  ...
  "message": "フィードバック。まず回答の良い点を褒める（共感）。その上で次の質問へ（メンター口調、250文字以内）",
  ...
  function buildContext(aspects, currentAspect, aspectStatus, aspectReason, aspectAdvice) {
    ...
    for (const [key, text] of Object.entries(aspects)) {
      ...
      if (key === currentAspect) {
        ...
        const preview = text?.trim() ? `「${text.trim().substring(0, 80)}」` : '';
        ctx += `${key}: ${st} ${preview} \n`;
      }
    }
    return ctx;
