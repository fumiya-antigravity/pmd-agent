...
### 【最重要】現在のタスク（index: ${currentTaskIndex || 0}）の処理
ユーザーから回答（スライダー入力）が得られたため、このタスクは**完了**したとみなす。
- **status**: 必ず "done" に設定せよ（"pending"や"retry"は禁止）。

### 次のステップ（新しいタスクの生成）
Whyの解像度がまだ低い場合（score < 80%）、次の深掘りタスクを1つだけ生成せよ。

**【重要】仮説（Options）の生成ルール**:
1. **重複禁止**: 前回までのタスクで提示したoptionsと同じ内容・同じ切り口の仮説を提示してはならない。全く新しい仮説を3つ提示せよ。
2. **多角的な視点**: 異なる角度からの仮説（例：前回が「リソース」なら今回は「組織構造」「形骸化したプロセス」など）を提示せよ。
3. **question_type**: 必ず "scale" とせよ。

- **completed**: \`why_completeness_score\` が **80%以上** になったら、これ以上質問を生成せず、新しいタスクのstatusを "completed" とせよ。
- **new**: まだ解像度が低い場合、上記ルールに従い新しい仮説検証タスクを生成せよ。
